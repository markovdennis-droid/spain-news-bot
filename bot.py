...
# ─── 9 категорий ─────────────────────────────────────────────────────────────
CATEGORIES = {
"politics": {"emoji": " ", "name": "Политика", "short": "Политика"},
"economy": {"emoji": " ", "name": "Экономика и бизнес", "short": "Экономика"},
"society": {"emoji": " ", "name": "Общество", "short": "Общество"},
"local": {"emoji": " ", "name": "Барселона / Мадрид", "short": "Локальные"},
"incidents": {"emoji": " ", "name": "Происшествия", "short": "Происшествия"},
"sports": {"emoji": " ", "name": "Спорт", "short": "Спорт"},
"culture": {"emoji": " ", "name": "Культура и афиша", "short": "Культура"},
"celebrities": {"emoji": " ", "name": "Знаменитости", "short": "Знаменитости"},
"humor": {"emoji": " ", "name": "Юмор и курьёзы", "short": "Юмор"},
}
ALL_CATEGORY_KEYS = list(CATEGORIES.keys())
# ─── RSS-источники ───────────────────────────────────────────────────────────
RSS_FEEDS = [
# Политика
("El País — España", "https://feeds.elpais.com/mrss-s/pages/ep/site/elpais.com/section/es
...
("El País — Internacional", "https://feeds.elpais.com/mrss-s/pages/ep/site/elpais.com/sec
# Экономика
("El País — Economía", "https://feeds.elpais.com/mrss-s/pages/ep/site/elpais.com/section/
...
# Общество
("El País — Sociedad", "https://feeds.elpais.com/mrss-s/pages/ep/site/elpais.com/section/
("20 Minutos — Sociedad", "https://www.20minutos.es/rss/sociedad/", "society"),
("El País — Educación", "https://feeds.elpais.com/mrss-s/pages/ep/site/elpais.com/section
("El País — Salud", "https://feeds.elpais.com/mrss-s/pages/ep/site/elpais.com/section/sal
# Локальные
("El País — Madrid", "https://feeds.elpais.com/mrss-s/pages/ep/site/elpais.com/section/ma
("El País — Catalunya", "https://feeds.elpais.com/mrss-s/pages/ep/site/elpais.com/section
...
("El País — Deportes", "https://feeds.elpais.com/mrss-s/pages/ep/site/elpais.com/section/
# Культура
("El País — Cultura", "https://feeds.elpais.com/mrss-s/pages/ep/site/elpais.com/section/c
...
""")
conn.execute("""
CREATE INDEX IF NOT EXISTS idx_sent_news_date ON sent_news(date_added)
""")
conn.commit()
conn.close()
...
conn = sqlite3.connect(DB_PATH)
conn.execute("UPDATE users SET digest_hour = ?, digest_minute = ? WHERE chat_id = ?", (ho
conn.commit()
conn.close()
def get_user_time(chat_id: int) -> tuple[int, int]:
conn = sqlite3.connect(DB_PATH)
row = conn.execute("SELECT digest_hour, digest_minute FROM users WHERE chat_id = ?", (cha
conn.close()
return row if row else (8, 0)
def get_user_subs(chat_id: int) -> list[str]:
conn = sqlite3.connect(DB_PATH)
row = conn.execute("SELECT subscriptions FROM users WHERE chat_id = ?", (chat_id,)).fetch
conn.close()
if row and row[0]:
try:
...
conn = sqlite3.connect(DB_PATH)
conn.execute("UPDATE users SET subscriptions = ? WHERE chat_id = ?", (json.dumps(subs), c
conn.commit()
conn.close()
...
conn = sqlite3.connect(DB_PATH)
rows = conn.execute(
"SELECT chat_id, subscriptions FROM users WHERE digest_hour = ? AND digest_minute = ?
(hour, minute),
).fetchall()
conn.close()
result = []
...
conn = sqlite3.connect(DB_PATH)
rows = conn.execute("SELECT category, digest_text FROM digest_cache WHERE date = ?", (dat
conn.close()
...
conn.execute("DELETE FROM sent_news WHERE date_added < date('now', '-14 days')")
conn.commit()
conn.close()
# ─── Дедупликация новостей ──────────────────────────────────────────────────
def normalize_title(title: str) -> str:
"""Нормализация заголовка для сравнения: lowercase, без знаков препинания, без коротких с
title = title.lower()
title = re.sub(r"[^\w\s]", "", title)
# Убираем стоп-слова (артикли, предлоги на испанском)
stop_words = {"el", "la", "los", "las", "un", "una", "de", "del", "en", "y", "a", "por",
...
"""Проверяет, похожи ли два заголовка по пересечению ключевых слов."""
words1 = set(normalize_title(title1).split())
words2 = set(normalize_title(title2).split())
if not words1 or not words2:
return False
intersection = words1 & words2
# Считаем по отношению к меньшему заголовку (чтобы короткий подзаголовок тоже ловился)
smaller = min(len(words1), len(words2))
if smaller == 0:
return False
return len(intersection) / smaller >= threshold
def is_news_duplicate(url: str, title: str, category: str) -> bool:
"""Проверяет, была ли эта новость уже отправлена (по URL или по заголовку)."""
conn = sqlite3.connect(DB_PATH)
# 1. Точное совпадение URL
row = conn.execute(
"SELECT COUNT(*) FROM sent_news WHERE url = ?", (url,)
).fetchone()
if row[0] > 0:
conn.close()
return True
# 2. Проверка по похожести заголовка (в той же категории, за последние 7 дней)
rows = conn.execute(
"SELECT title_normalized FROM sent_news WHERE category = ? AND date_added >= date('no
(category,),
).fetchall()
conn.close()
norm_title = normalize_title(title)
for (prev_title,) in rows:
if titles_are_similar(norm_title, prev_title, threshold=0.55):
return True
return False
def save_sent_news(entries: list[tuple[str, str, str]]):
"""Сохраняет отправленные новости. entries = [(url, title, category), ...]"""
today = datetime.now(TIMEZONE).strftime("%Y-%m-%d")
conn = sqlite3.connect(DB_PATH)
for url, title, category in entries:
norm = normalize_title(title)
conn.execute(
"INSERT OR IGNORE INTO sent_news (url, title_normalized, category, date_added) VA
(url, norm, category, today),
)
conn.commit()
conn.close()
# ─── RSS-парсер ──────────────────────────────────────────────────────────
...
accepted_entries: list[tuple[str, str, str]] = [] # (url, title, category)
...
# Очистка HTML
summary = re.sub(r"<[^>]+>", " ", summary) # теги → пробел
summary = re.sub(r"&\w+;", " ", summary) # &nbsp; и т.д.
summary = re.sub(r"\s+", " ", summary).strip() # лишние пробелы
title = re.sub(r"<[^>]+>", "", title).strip()
# Пропускаем пустые или мусорные записи
if not title or len(title) < 10:
continue
if len(summary) > 400:
summary = summary[:400] + "..."
# ── Дедупликация: пропускаем уже отправленные новости ──
if is_news_duplicate(link, title, category):
...
accepted_entries.append((link, title, category))
except Exception as e:
logger.warning(f"Ошибка RSS {feed_name}: {e}")
# Сохраняем принятые новости как "уже отправленные"
if accepted_entries:
save_sent_news(accepted_entries)
logger.info(f" Принято новостей: {len(accepted_entries)}, дубли отфильтрованы")
return {cat: "\n\n---\n\n".join(entries) for cat, entries in news_by_cat.items() if entri
# ─── Claude API ──────────────────────────────────────────────────────────────
CATEGORY_PROMPTS = {
"politics": """Раздел: *ПОЛИТИКА*
Подтемы: национальная политика Испании, взаимоотношения с ЕС и миром, решения правительства,
Стиль: серьёзный, без юмора. 2-4 главных новости, каждая 1-2 предложения.""",
"economy": """Раздел: *ЭКОНОМИКА И БИЗНЕС*
...
"culture": """Раздел: *КУЛЬТУРА И АФИША*
...
"celebrities": """Раздел: *ЗНАМЕНИТОСТИ*
...
КРИТИЧЕСКИ ВАЖНО:
- Включай ТОЛЬКО новости, которые касаются ИСПАНИИ или непосредственно затрагивают Испанию.
- ИГНОРИРУЙ новости о других странах (Британия, США, Россия и т.д.) если они не связаны с Исп
- Если в исходных материалах есть HTML-код, мусор, нечитаемый текст — ИГНОРИРУЙ это полностью
ПРАВИЛА:
...
- Приоритет: 1) новости из Испании, 2) новости об испанцах за рубежом (спортсмены, компании,
"""
...
# Нет RSS-новостей — просим Claude написать анонс/факт
text = await generate_category_digest(
category,
"Сегодня свежих новостей по этой теме нет. "
"Напиши краткий анонс ближайшего события в Испании по этой теме, "
"или интересный факт об Испании связанный с этой темой."
)
except Exception as e:
...
parts = [f" *Испания Daily* — {today_display}\n"]
...
# ─── Отправка ────────────────────────────────────────────────────────────────
async def send_digest_message(chat_id: int, text: str, context: ContextTypes.DEFAULT_TYPE) ->
try:
if len(text) > 4096:
parts = [text[i : i + 4096] for i in range(0, len(text), 4096)]
for part in parts:
await context.bot.send_message(
chat_id=chat_id, text=part, parse_mode="Markdown", disable_web_page_previ
)
else:
await context.bot.send_message(
chat_id=chat_id, text=text, parse_mode="Markdown", disable_web_page_preview=T
)
return True
except Exception as e:
logger.error(f" return False
Ошибка отправки → {chat_id}: {e}")
# ─── Закреплённое меню ───────────────────────────────────────────────────────
MENU_TEXT = (
" *Меню Испания Daily*\n\n"
" /topics — Выбрать темы\n"
...
"SELECT digest_hour, COUNT(*) as cnt FROM users WHERE is_active = 1 GROUP BY digest_h
).fetchall()
top_times = "\n".join(f" {h:02d}:00 — {c} чел." for h, c in time_rows) if time_rows else
...
conn = sqlite3.connect(DB_PATH)
conn.execute("UPDATE users SET is_active = 0 WHERE chat_id = ?", (update.effective_user.i
conn.commit()
conn.close()
await update.message.reply_text(" Рассылка приостановлена.\n/resume — возобновить.")
async def cmd_resume(update: Update, context: ContextTypes.DEFAULT_TYPE):
conn = sqlite3.connect(DB_PATH)
conn.execute("UPDATE users SET is_active = 1 WHERE chat_id = ?", (update.effective_user.i
conn.commit()
conn.close()
hour, minute = get_user_time(update.effective_user.id)
await update.message.reply_text(
f" Возобновлено! Дайджест в *{hour:02d}:{minute:02d}*",
parse_mode="Markdown",
)
# ─── Клавиатуры ──────────────────────────────────────────────────────────────
...
Дайджест каждый день в *{hour:02d}:{minute:02d}* (Мадрид)",
)
# ── Темы ──
...
if not users:
return
logger.info(f" for chat_id, subs in users:
if subs:
{now.strftime('%H:%M')} — рассылка {len(users)} пользователям")
