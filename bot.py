...
CATEGORIES = {
    ...
    "sports":      {"emoji": "⚽️", "name": "Спорт",              "short": "Спорт"},
    "culture":     {"emoji": "🎭", "name": "Культура и афиша",   "short": "Культура"},
    "celebrities": {"emoji": "⭐️", "name": "Знаменитости",      "short": "Знаменитости"},
...
RSS_FEEDS = [
    ...
    ("El País — Catalunya",   "https://feeds.elpais.com/mrss-s/pages/ep/site/elpais.com/section/cataluna/portada", "local"),
...
def init_db():
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_sent_news_url ON sent_news(url)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_sent_news_date ON sent_news(date_added)")
    conn.commit()
    conn.close()
...
def has_today_digest() -> bool:
    ...
    conn = sqlite3.connect(DB_PATH)
    # Считаем только нормальные записи (без заглушки "временно недоступен")
    row = conn.execute(
        "SELECT COUNT(*) FROM digest_cache WHERE date = ? AND digest_text NOT LIKE '%временно недоступен%'",
        (today,)
    ).fetchone()
    conn.close()
    return row[0] >= len(CATEGORIES)  # все 9 категорий должны быть готовы
...
def normalize_title(title: str) -> str:
    ...
    stop_words = {"el", "la", "los", "las", "un", "una", "de", "del", "en", "y", "a", "por", "que", "se", "su"}
...
def is_news_duplicate(url: str, title: str, category: str) -> bool:
    conn = sqlite3.connect(DB_PATH)
    row = conn.execute("SELECT COUNT(*) FROM sent_news WHERE url = ?", (url,)).fetchone()
    if row[0] > 0:
        conn.close()
        return True
    rows = conn.execute(
        "SELECT title_normalized FROM sent_news WHERE category = ? AND date_added >= date('now', '-7 days')",
        (category,),
    ).fetchall()
    conn.close()
...
def save_sent_news(entries: list[tuple[str, str, str]]):
    ...
    for url, title, category in entries:
        ...
        conn.execute(
            "INSERT OR IGNORE INTO sent_news (url, title_normalized, category, date_added) VALUES (?, ?, ?, ?)",
...
def fetch_news_by_category() -> dict[str, str]:
    ...
    accepted_entries: list[tuple[str, str, str]] = []
...
- ИГНОРИРУЙ новости о других странах если они не связаны с Испанией.
...
- Приоритет: 1) новости из Испании, 2) новости об испанцах за рубежом (спортсмены, компании, политики).
"""

ANTHROPIC_MODELS = [
    os.environ.get("ANTHROPIC_MODEL", "claude-sonnet-4-6"),  # основная — берётся из Railway
    "claude-sonnet-4-6",      # fallback 1
    "claude-sonnet-4-5",      # fallback 2
    "claude-haiku-4-5-20251001",  # fallback 3 (самый лёгкий)
]
# Убираем дубли, сохраняя порядок
ANTHROPIC_MODELS = list(dict.fromkeys(ANTHROPIC_MODELS))
...
async def generate_category_digest(category: str, news_text: str) -> str:
    ...
    for model in ANTHROPIC_MODELS:
        try:
            ...
            err = str(e).lower()
            if "model_not_found" in err or "invalid model" in err or "not_found_error" in err:
                logger.warning(f"⚠️ Модель {model} недоступна, пробую следующую...")
                continue
            raise  # другие ошибки (сеть, токены и т.д.) — пробрасываем сразу
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=chat_id,
        text=(
            "☝️ Это был полный дайджест из 9 разделов.\n\n"
...
async def cmd_refresh(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Очистить кеш сегодняшнего дня и перегенерировать — только для админа."""
    if update.effective_user.id != ADMIN_ID:
        return
    today = datetime.now(TIMEZONE).strftime("%Y-%m-%d")
    conn = sqlite3.connect(DB_PATH)
    conn.execute("DELETE FROM digest_cache WHERE date = ?", (today,))
    conn.commit()
    conn.close()
    await update.message.reply_text("🗑 Кеш очищен. Генерирую заново, подожди 1-2 минуты...")
    await generate_all_digests()
    await update.message.reply_text("✅ Готово! Напиши /start чтобы получить свежий дайджест.")
...
def build_topics_keyboard(chat_id: int) -> InlineKeyboardMarkup:
    ...
    keyboard.append([
        ...
        InlineKeyboardButton("☑️ Сброс", callback_data="topic_none"),
    ])
    keyboard.append([InlineKeyboardButton("✔️ Готово", callback_data="topic_done")])
...
def build_time_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            ...
            InlineKeyboardButton("☀️ 09:00", callback_data="time_09_00"),
        ],
        [
